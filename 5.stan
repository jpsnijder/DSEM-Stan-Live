...
model {
  ...
  for (i in 1:N_subj) {
    ...
    real log_psi_X_2 = gamma[7] + f[7] * P[i] + u[i,7];
    real log_psi_Y_2 = gamma[8] + f[8] * P[i] + u[i,8];
    real psi_X = sqrt(exp(log_psi_X_2));
    real psi_Y = sqrt(exp(log_psi_Y_2));
    ...
    O[i] ~ normal(
      tau[1] + tau[2] * P[i] + tau[3] * mu_X + tau[4] * mu_Y +
      tau[5] * phi_X + tau[6] * phi_Y + tau[7] * phi_XY +
      tau[8] * beta_YX + tau[9] * log_psi_X_2 + tau[10] * log_psi_Y_2,
      sigma_O
    );
  }
}
