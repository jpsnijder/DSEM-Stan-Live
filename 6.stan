...
model {
  ...
  for (i in 1:N_subj) {
    ...
    real mu_X = gamma[1] + f_[1] * C_[i] + u[i,1];
    real mu_Y = gamma[2] + f_[2] * C_[i] + u[i,2];
    
    real phi_X = gamma[3] + f_[3] * C_[i] + u[i,3];
    real phi_Y = gamma[4] + f_[4] * C_[i] + u[i,4];
    real phi_XY = gamma[5] + f_[5] * C_[i] + u[i,5];
    real beta_YX = gamma[6] + f_[6] * C_[i] + u[i,6];
    
    real psi_X = sqrt(exp(gamma[7] + f_[7] * C_[i] + u[i,7]));
    real psi_Y = sqrt(exp(gamma[8] + f_[8] * C_[i] + u[i,8]));
